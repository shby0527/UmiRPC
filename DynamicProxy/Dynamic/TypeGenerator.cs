...
internal class TypeGenerator
{
    ...
    private static readonly MethodInfo MethodGetMethodFromHandle;

    private static readonly MethodInfo ParentMethod;

    private static readonly ConstructorInfo ParentConstructor;

    private static readonly MethodInfo ParentStaticMethod;
    ...
    private static readonly ConstructorInfo AttributeConstructor;

    #region Event 相关字段

    private static readonly MethodInfo EventSubscribe;
    private static readonly MethodInfo EventUnsubscribe;


    private static readonly MethodInfo DelegateCombine;
    private static readonly MethodInfo DelegateRemove;

    private static readonly MethodInfo RaiseEvent;
    ...
    static TypeGenerator()
    {
        ...
        MethodGetMethodFromHandle = typeof(MethodBase).GetMethod(nameof(MethodBase.GetMethodFromHandle),
                                        BindingFlags.Static | BindingFlags.Public,
                                        [typeof(RuntimeMethodHandle), typeof(RuntimeTypeHandle)])
                                    ?? throw new InvalidOperationException("can not found helper method");

        AttributeConstructor = typeof(AutoGeneratedGuidAttribute)
        ...
        ParentMethod =
            ...
            ?? throw new InvalidOperationException("can not found helper method");
        ParentStaticMethod = typeof(AutoGeneratedBase).GetMethod("ProcessStatic",
        ...
        EventSubscribe = typeof(AutoGeneratedBase)
                             .GetMethod("EventSubscribe",
                                 BindingFlags.Instance | BindingFlags.NonPublic,
                                 [typeof(string)])
                         ?? throw new InvalidOperationException("can not found helper method");

        EventUnsubscribe = typeof(AutoGeneratedBase)
                               .GetMethod("EventUnsubscribe",
                                   BindingFlags.Instance | BindingFlags.NonPublic,
                                   [typeof(string)])
                           ?? throw new InvalidOperationException("can not found helper method");

        DelegateCombine = typeof(Delegate)
                              .GetMethod(nameof(Delegate.Combine),
                                  [typeof(Delegate), typeof(Delegate)])
                          ?? throw new InvalidOperationException("can not found helper method");

        DelegateRemove = typeof(Delegate)
                             .GetMethod(nameof(Delegate.Remove),
                                 [typeof(Delegate), typeof(Delegate)])
                         ?? throw new InvalidOperationException("can not found helper method");

        RaiseEvent = typeof(AutoGeneratedBase)
        ...
        EventInfo eventInfo)
    {
        ...
        {
            case 1:
                il.Emit(OpCodes.Call, EventSubscribe);
                break;
            case 2:
                il.Emit(OpCodes.Call, EventUnsubscribe);
                break;
            default:
                il.Emit(OpCodes.Call, EventSubscribe);
                break;
    ...
    {
        var parameters = RaiseEvent.GetParameters().Select(p => p.ParameterType).ToArray();
        var attribute = RaiseEvent.Attributes ^ MethodAttributes.Abstract;
        var opEqual = typeof(string).GetMethod("op_Equality", [typeof(string), typeof(string)])!;

        var methodBuilder = _typeBuilder.DefineMethod(RaiseEvent.Name,
            attribute,
            RaiseEvent.CallingConvention, RaiseEvent.ReturnType, parameters);
        methodBuilder.SetReturnType(RaiseEvent.ReturnType);
        _typeBuilder.DefineMethodOverride(methodBuilder, RaiseEvent);
    ...
    private void MethodImplement(MethodBuilder methodBuilder, MethodInfo origin)
    {
        ...
        il.Emit(OpCodes.Call, MethodGetMethodFromHandle);
        ...
        if (origin.IsStatic)
        {
            ...
            il.Emit(OpCodes.Call, ParentStaticMethod);
        }
        else
        {
            il.Emit(OpCodes.Ldarg_0);
